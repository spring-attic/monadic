...
(function () {
    ...
    var parserDo = require('./javascript_do');
    var parser = require('./javascript_plain');
    ...
    function TreeWalker(ast, recv, parent, name) {
        var self = this;
        this.ast = ast;
        this.recv = recv;
        this.stack =
            [{traverse: ast, parent: parent, name: name},
             {fun: function () {
    ...
    PrettyPrinter.prototype = {
        ...
        BinaryExpression: function (tw, parent, obj, name) {
            ...
            tw.invoke(function () {
                var lhs, rhs;
                rhs = self.text.join("");
                if (obj.right.type === "BinaryExpression") {
                    rhs = "(" + rhs + ")";
                }
                self.popScope();
                lhs = self.text.join("");
                if (obj.left.type === "BinaryExpression") {
                    lhs = "(" + lhs + ")";
                }
                self.popScope();
        ...
        DoSyntaxStatement: function (tw, parent, obj, name) {
            var self = this;
            tw.invoke(function () {
                var monad, statements;
                monad = self.text.join("");
                if (obj.monad.type === "BinaryExpression") {
                    monad = "(" + monad + ")";
                }
                self.popScope();
                statements = self.text.join("");
                self.popScope();
                self.withFreshScopeJoin(function () {
                    self.append("do " + monad);
                    if ("Block" === obj.statement.type) {
                        self.append(" ");
                        self.needsTermination = false;
                    } else {
        ...
        VariableDeclaration: function (tw, parent, obj, name) {
            ...
            if (obj.value) {
                tw.invoke(function () {
                    ...
                    self.append(obj.name + " " + obj.operator + " " + value);
                });
                tw.walk('value');
                this.freshScope();
            } else {
    ...
    function Binder(monad, statement) {
        this.monad = monad;
        if ('Block' === statement.type) {
            this.statementsOrig = statement.statements;
        } else {
            this.statementsOrig = [statement];
        }
    }

    Binder.prototype = {

        transform: function (parse) {
            this.statements = [].concat(this.statementsOrig);
            this.idx = 0;
            this.text = [];
            this.next();
            this.text = '(function () { ' + this.text.join(' ') + ' }.bind(this)())';
            if (parse) {
                return parser.parse(this.text).elements[0];
            } else {
                return this.text;
            }
        },
        ...
        next: function () {
            while (this.idx < this.statements.length) {
                this.idx += 1;
                new TreeWalker(this.statements[this.idx - 1], this, this.statements, this.idx - 1).traverse();
            }
        },

        ReturnStatement: function (tw, parent, obj, name) {
            this.append('return ' + this.monad + '.mreturn(' +
                        (obj.value ? new PrettyPrinter(obj.value).print() : '') +
                        ');');
        },

        FunctionCall: function (tw, parent, obj, name) {
            var funName = new PrettyPrinter(obj.name).print(),
                call = new PrettyPrinter(obj).print();
            if ('fail' === funName) {
                this.append('return ' + this.monad + '.' + call + ';');
            } else if (Array.isArray(parent) && name + 1 === parent.length) {
                this.append('return ' + call + ';');
            } else {
                this.append('return ' + this.monad + '.mbind(' + call + ', function () { ');
                this.next();
        ...
        AssignmentExpression: function (tw, parent, obj, name) {
            var value, left;
            if ('<-' === obj.operator) {
                value = new PrettyPrinter(obj.right).print();
                left =  new PrettyPrinter(obj.left).print();
                this.append('return ' + this.monad + '.mbind(' + value + ', function (' + left + ') { ');
                this.next();
                this.append(' }.bind(this)); ');
            } else {
                this.append(new PrettyPrinter(obj).print() + ';');
                this.next();
            }
        },
        ...
        VariableStatement: function (tw, parent, obj, name) {
            var i;
            for (i = obj.declarations.length - 1; i >= 0; i -= 1) {
                this.statements.splice(this.idx, 0, obj.declarations[i]);
            }
            this.next();
        },
        ...
        VariableDeclaration: function (tw, parent, obj, name) {
            ...
            if ('<-' === obj.operator || undefined === obj.operator) {
                if (obj.value) {
                    value = new PrettyPrinter(obj.value).print();
                }
                this.append('return ' + this.monad + '.mbind(' + value + ', function (' + obj.name + ') { ');
                this.next();
                this.append(' }.bind(this)); ');
            } else {
                this.append('var ' + (new PrettyPrinter(obj).print()) + ';');
                this.next();
            }
        },

        IfStatement: function (tw, parent, obj, name) {
            this.append('if (' + (new PrettyPrinter(obj.condition).print()) + ') { ');
            this.append('return ' + new Binder(this.monad, obj.ifStatement).transform(false));
            if (obj.elseStatement) {
                this.append(' } else { ');
                this.append('return ' + new Binder(this.monad, obj.elseStatement).transform(false));
            }
            this.append(' }');
        }
    };
    ...
    (function () {
        var keys, key, i, len, letter;
        keys = Object.keys(PrettyPrinter.prototype);
        len = keys.length;
        for (i = 0; i < len; i += 1) {
            key = keys[i];
            letter = key.charAt(0);
            if (letter === letter.toUpperCase() && ! (key in Binder.prototype)) {
                Binder.prototype[key] = function (tw, parent, obj, name) {
                    this.append(new PrettyPrinter(obj).print() + ';');
                };
            }
        }
    ...
    DoSyntax.prototype = {
        ...
        DoSyntaxStatement: function (tw, parent, obj, name) {
            var self = this;
            tw.invoke(function () {
                var monad, statements, binder;
                monad = new PrettyPrinter(obj.monad).print();
                parent[name] = new Binder(monad, obj.statement).transform(true);
            });
            tw.walk('statement');
            tw.walk('monad');
        }
    ...
    exports.parseDo = parserDo.parse;
    exports.TreeWalker = TreeWalker;
    exports.PrettyPrinter = PrettyPrinter;
    exports.DoSyntax = DoSyntax;

    exports.test = function () {
        var txt = require('fs').readFileSync(__dirname + '/../test/syntax-test.js', 'utf8');
        console.log("Input:", txt);
        var ast = parserDo.parse(txt);
        var ds = new DoSyntax(ast);
        ds.transform();
        var pp = new PrettyPrinter(ds.ast);
        console.log("Output:", pp.print());
    }
}());
