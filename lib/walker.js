...
(function () {
    ...
    // Hacks
    PrettyPrinter.prototype.DoAssignmentExpression =
        PrettyPrinter.prototype.AssignmentExpression;
    PrettyPrinter.prototype.ThrowExpressionStatement =
        PrettyPrinter.prototype.ThrowStatement;
    ...
    DoSyntax.prototype = {
        ...
        setMonad: function (monad) {
            this.monads.push(this.monad);
            this.monad = monad;
        },

        popMonad: function () {
            this.monad = this.monads.pop();
        },

        DoSyntaxStatement: function (tw, parent, obj, name) {
            if ('EmptyStatement' === obj.statement.type) {
                throw 'A do-block may not be empty';
            }
            tw.invoke(function () {
                parent[name] = obj.statement;
                this.popMonad();
            }.bind(this));
            tw.walk('statement');
            tw.invoke(function () {
                this.setMonad(new PrettyPrinter(obj.monad).print());
            }.bind(this));
            tw.walk('monad');
        },

        DoReturnExpression: function (tw, parent, obj, name) {
            tw.invoke(function () {
                var str = '(function () { return ' + this.monad + '.mreturn(' +
                    (obj.value ? new PrettyPrinter(obj.value).print() : '') +
                    '); }.bind(this)())';
                parent[name] = parser.parse(str).elements[0];
            }.bind(this));
            if (obj.value) {
                tw.walk('value');
            }
        ...
        DoIfExpression: function (tw, parent, obj, name) {
            tw.invoke(function () {
                var str = '(function () { if (' +
                    new PrettyPrinter(obj.condition).print() + ') { return ' +
                    new PrettyPrinter(obj.ifStatement).print() + '; } else { return ' +
                    new PrettyPrinter(obj.elseStatement).print() + '; } }.bind(this)())'
                parent[name] = parser.parse(str).elements[0];
            }.bind(this));
            tw.walk('elseStatement');
            tw.walk('ifStatement');
        ...
        ThrowExpressionStatement: function (tw, parent, obj, name) {
            tw.invoke(function () {
                var str = '(function () { ' + new PrettyPrinter(obj).print() + '}.bind(this)())';
                parent[name] = parser.parse(str).elements[0];
            }.bind(this));
            tw.walk('exception');
        },

        DoBlock: function (tw, parent, obj, name) {
            var len = obj.statements.length,
                last, idx, len, statement, acc, param;
            if (0 === len) {
                throw 'A do-block may not be empty';
            }
            last = obj.statements[len - 1];
            if ('DoAssignmentExpression' === last.type) {
                throw 'The last statement in a do-block must be an expression';
            }
            tw.invoke(function () {
                acc = '(' + new PrettyPrinter(obj.statements.pop()).print() + ')';
                while (0 !== obj.statements.length) {
                    statement = obj.statements.pop();
                    if ('DoAssignmentExpression' === statement.type) {
                        if ('<-' === statement.operator) {
                            param = new PrettyPrinter(statement.left).print();
                            statement = statement.right;
                            acc = this.monad + '.mbind(' + new PrettyPrinter(statement).print() +
                                ', function (' + param + ') { return ' + acc + '; }.bind(this))';
                        } else {
                            param = '';
                            statement.type = 'AssignmentExpression';
                            acc = '(function () { ' + new PrettyPrinter(statement).print() +
                                '; return ' + acc + '; }.bind(this)())';
                        }
                    } else {
                        acc = this.monad + '.mbind(' + new PrettyPrinter(statement).print() +
                            ', function () { return ' + acc + '; }.bind(this))';
                    }
                }
                parent[name] = parser.parse(acc).elements[0];
            }.bind(this));
            for (idx = 0; idx < len; idx += 1) {
                tw.walk(obj.statements, idx);
            }
        }
    ...
    exports.test = function () {
        var txt = require('fs').readFileSync(__dirname + '/../test/syntax-test.mjs', 'utf8');
