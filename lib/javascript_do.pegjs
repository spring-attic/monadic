...
VariableDeclaration
  = name:Identifier __ initialiser:Initialiser? {
      return {
...
VariableDeclarationNoIn
  = name:Identifier __ initialiser:InitialiserNoIn? {
      return {
        type:     "VariableDeclaration",
        name:     name,
        operator: initialiser.operator !== "" ? initialiser.operator : null,
...
Initialiser
  = operator:InitialiserOperator __ expression:AssignmentExpression {
      return {
        operator: operator,
        value:    expression
      };
...
InitialiserNoIn
  = operator:InitialiserOperator __ expression:AssignmentExpressionNoIn {
      return {
        operator: operator,
        value:    expression
      };
...
DoStatement
  ...
  / ThrowExpressionStatement
  / FunctionExpression

DoBlock
  = "{" __ statements:(DoStatementList __)? "}" {
      return {
        type:       "DoBlock",
        statements: statements !== "" ? statements[0] : []
      };
...
DoStatementList
  = head:DoStatement tail:(__ DoStatement)* {
      var result = [head];
      for (var i = 0; i < tail.length; i++) {
        result.push(tail[i][1]);
      }
...
DoExpressionStatement
  = !("{" / FunctionToken) expression:DoExpression EOS { return expression; }

DoExpression
  = head:DoAssignmentExpression
    tail:(__ "," __ DoAssignmentExpression)* {
      var result = head;
      for (var i = 0; i < tail.length; i++) {
        result = {
...
DoAssignmentExpression
  = left:LeftHandSideExpression __
    operator:AssignmentOperator __
    right:DoAssignmentExpression {
      return {
...
DoIfExpression
  = IfToken __
    "(" __ condition:Expression __ ")" __
    ifStatement:DoStatement __
    ElseToken __
    elseStatement:DoStatement {
      return {
        type:          "DoIfExpression",
        condition:     condition,
        ifStatement:   ifStatement,
...
ReturnExpression
  = ReturnToken value:(__ DoExpression)? {
      return {
