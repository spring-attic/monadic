(function () {
    ...
    exports.monad = function () {
        ...
        // Note that the stateT(contT(m)) monad's `suspend`
        // continuation function can take an optional second
        // argument. The first argument, as normal, is the value to
        // provide back to the continuation itself. The second is the
        // state that you wish the continuation to carry on from. If
        // the state is not provided, the *original* state in use when
        // `suspend` was first invoked will be used. This detail is
        // only really of note if you're planning on invoking the
        // continuation multiple times and you want to ensure state is
        // chained through.
